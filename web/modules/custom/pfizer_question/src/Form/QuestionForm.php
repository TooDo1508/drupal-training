...
class QuestionForm extends FormBase
{
    ...
    public function submitForm(array &$form, FormStateInterface $form_state)
    {
        ...
        $convert_cb = implode(", ", array_diff($form_state->getValue('question_3a'), array('0')));
        $page_values = $form_state->get('page_values');
        $form_state->set('page_values', [
            'question_3' => $form_state->getValue('question_3'),
            'question_3a' => $convert_cb,
            'question_2' => $page_values['question_2'],
...
//        }
        ...
        switch ($page_num) {
            case 1:
                if ($form_state->getValue('question_1') == NULL) {
                    $form_state->set('page_values', ['question_1' => ""]);
                }
                $form_state->set('page_num', $page_num + 1)->setRebuild(TRUE);
                break;
    ...
    public function subscribePageThree(array &$form, FormStateInterface $form_state)
    {
        ...
        $form['question_3'] = array(
            '#type' => 'radios',
            '#default_value' => $form_state->getValue('question_3', ''),
            '#options' => [
                'Yes' => $this->t('Yes'),
                'No' => $this->t('No'),
            ],
        ...
        $form['question_3a'] = array(
            '#type' => 'checkboxes',
            '#default_value' => $form_state->getValue('question_3', ''),
            '#options' => [
                'Diarrhea' => $this->t('Diarrhea'),
                'Blood in stool' => $this->t('Blood in stool'),
                'Change in frequency of bowel movements' => $this->t('Change in frequency of bowel movements'),
                'Increased stool urgency and/or accidents' => $this->t('Increased stool urgency and/or accidents'),
            ],
        );
